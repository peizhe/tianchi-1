...
drop table if exists new.train_user_l1_cart;
create table new.train_user_l1_cart as
    select user_id,ifnull(user_l1_cart,0) as user_l1_cart from
    (select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,count(behavior_type) as user_l1_cart from washed.tianchi_p_ten_11_30 
    where (behavior_type=3) and (event_date=30) group by user_id)b
    on (a.user_id=b.user_id_1);


drop table if exists new.train_user_l3_cart;
create table new.train_user_l3_cart as
    select user_id,ifnull(user_l3_cart,0) as user_l3_cart from
    (select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,count(behavior_type) as user_l3_cart from washed.tianchi_p_ten_11_30 
    where (behavior_type=3) and (event_date between 28 and 30) group by user_id)b
    on (a.user_id=b.user_id_1);


drop table if exists new.train_user_l7_cart;
create table new.train_user_l7_cart as
    select user_id,ifnull(user_l7_cart,0) as user_l7_cart from
    (select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,count(behavior_type) as user_l7_cart from washed.tianchi_p_ten_11_30 
    where (behavior_type=3) and (event_date between 26 and 30 or event_date between 22 and 23) group by user_id)b
    on (a.user_id=b.user_id_1);


drop table if exists new.train_user_l7_favor;
create table new.train_user_l7_favor as
    select user_id,ifnull(user_l7_favor,0) as user_l7_favor from
    (select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,count(behavior_type) as user_l7_favor from washed.tianchi_p_ten_11_30 
    where (behavior_type=2) and (event_date between 26 and 30 or event_date between 22 and 23) group by user_id)b
    on (a.user_id=b.user_id_1);


drop table if exists new.train_user_l10_favor;
create table new.train_user_l10_favor as
    select user_id,ifnull(user_l10_favor,0) as user_l10_favor from
    (select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,count(behavior_type) as user_l10_favor from washed.tianchi_p_ten_11_30 
    where (behavior_type=2) and (event_date between 26 and 30 or event_date between 19 and 23) group by user_id)b
    on (a.user_id=b.user_id_1);

drop table if exists new.train_user_l14_favor;
create table new.train_user_l14_favor as
    select user_id,ifnull(user_l14_favor,0) as user_l14_favor from
    (select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,count(behavior_type) as user_l14_favor from washed.tianchi_p_ten_11_30 
    where (behavior_type=2) and (event_date between 26 and 30 or event_date between 15 and 23) group by user_id)b
    on (a.user_id=b.user_id_1);
...
drop table if exists new.train_user_l1_distinct_clk;
create table new.train_user_l1_distinct_clk as
...
drop table if exists new.train_user_l3_distinct_clk;
create table new.train_user_l3_distinct_clk as
...
drop table if exists new.train_user_l7_distinct_clk;
create table new.train_user_l7_distinct_clk as
...
drop table if exists new.train_user_l1_distinct_buy;
create table new.train_user_l1_distinct_buy as
...
drop table if exists new.train_user_l3_distinct_buy;
create table new.train_user_l3_distinct_buy as
...
drop table if exists new.train_user_l7_distinct_buy;
create table new.train_user_l7_distinct_buy as
...
(select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join
(select user_id as user_id_1,31-max(event_date) as user_l_buy_date from washed.tianchi_p_ten_11_30 
...
(select distinct user_id from washed.tianchi_p_ten_11_30)a
    left outer join 
(select user_id as user_id_1,31-max(event_date) as user_l_act_date from washed.tianchi_p_ten_11_30 
   group by user_id)b
   on (a.user_id=b.user_id_1);
...
drop table if exists new.train_ui_l7_cart;
create table new.train_ui_l7_cart as
    select user_id,item_id,ifnull(ui_l7_cart,0) as ui_l7_cart from
    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1, item_id as item_id_1, count(behavior_type) as ui_l7_cart from washed.tianchi_p_ten_11_30 
    where behavior_type=3 and (event_date between 26 and 30 or event_date between 22 and 23) group by user_id,item_id)b
    on (a.user_id=b.user_id_1 and a.item_id=b.item_id_1);
...
drop table if exists new.train_ui_l14_favor;
create table new.train_ui_l14_favor as
    select user_id,item_id,ifnull(ui_l14_favor,0) as ui_l14_favor from
    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1, item_id as item_id_1, count(behavior_type) as ui_l14_favor from washed.tianchi_p_ten_11_30 
    where behavior_type=2 and (event_date between 26 and 30 or event_date between 15 and 23) group by user_id,item_id)b
    on (a.user_id=b.user_id_1 and a.item_id=b.item_id_1);

drop table if exists new.train_ui_l18_favor;
create table new.train_ui_l18_favor as
    select user_id,item_id,ifnull(ui_l18_favor,0) as ui_l18_favor from
    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1, item_id as item_id_1, count(behavior_type) as ui_l18_favor from washed.tianchi_p_ten_11_30 
    where behavior_type=2 and (event_date between 26 and 30 or event_date between 11 and 23) group by user_id,item_id)b
    on (a.user_id=b.user_id_1 and a.item_id=b.item_id_1);
...
create table new.train_ui_l_act_date as 
    ...
    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,item_id as item_id_1,31-max(event_date) as ui_l_act_date from washed.tianchi_p_ten_11_30 
...
create table new.train_ui_f_buy_date as 
    ...
    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,item_id as item_id_1,31-min(event_date) as ui_f_buy_date from washed.tianchi_p_ten_11_30 
...
create table new.train_ui_l_buy_date as 
    ...
    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)a
    left outer join
    (select user_id as user_id_1,item_id as item_id_1,31-max(event_date) as ui_l_buy_date from washed.tianchi_p_ten_11_30 
    where behavior_type=4 group by user_id,item_id)b
    on (a.user_id=b.user_id_1 and a.item_id=b.item_id_1); 
...
drop table if exists new.train_ui_l_actnextday_other_distinct_act;
create table new.train_ui_l_actnextday_other_distinct_act as
select user_id,item_id,ifnull(user_ui_l_actnextday_distinct_act,0) as ui_l_actnextday_other_distinct_act from
    ...
    (
        select distinct user_id,item_id,event_date from washed.tianchi_p_ten_11_30 
        order by user_id,event_date
    )a
    right outer join
    (
        select user_id as user_id_1,item_id as item_id_1,ui_l_act_date 
        from new.train_ui_l_act_date
    )b
...
drop table if exists new.train_ui_l1_h1_only_act;
create table new.train_ui_l1_h1_only_act as

    select user_id,item_id,ifnull(ui_l1_h1_only_act,0) as  ui_l1_h1_only_act from

    (select distinct user_id,item_id from washed.tianchi_p_ten_11_30)aaa
...
drop table if exists new.train_ui_l3_h1_only_act;
create table new.train_ui_l3_h1_only_act as
